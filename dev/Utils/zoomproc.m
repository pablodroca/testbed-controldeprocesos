function [x,y,button] = ginput_zoom(f,a,n, style)
...
set(f,'pointer', style);  % use fullcross, user can change this
    ...
    while 1
        ...
        if sum(point1 == point2) == 2        % same points
            if  k == 1                       % detects a key press
                x(i) = point1(1);
                y(i) = point1(2);
                button(i) = double(get(f,'CurrentCharacter'));   % get keyboad ascii number
